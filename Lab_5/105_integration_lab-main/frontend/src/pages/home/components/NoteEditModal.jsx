...
const NoteEditModal = ({ note = {}, open = false, handleClose = () => {}, setNotes = () => {} }) => {
  ...
  const validateForm = () => {
    ...
    if (!newNote.description) error.description = 'Description is required'; setError(error);
    ...
    const submit = async () => {
      ...
      const userToken = Cookies.get('UserToken'); const response = await Axios.patch(
        '/note', {
              title: newNote.title,
      ...
      if (response.data.success) {
        setStatus({ severity: 'success', msg: 'Update note successfully' }); setNotes((prev) => prev.map((n) => (n.id === newNote.id ?
        response.data.data : n))); resetAndClose();
        }
      } catch (error) {
      // 4. if update note failed, check if error is from calling API or not 
      if (error instanceof AxiosError && error.response) {
      setStatus({ severity: 'error', msg: error.response.data.error }); } else {
